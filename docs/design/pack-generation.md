...
**Status (2026-06-06):** Phase 1 shipped. **Phase 2 deterministic core shipped + tested:** `src/packgen/gate.ts` (`gateCards` + `writePack`: substring honesty gate + `unverified-number`/`unverified-proper-noun` + `checkCardQuality` wired in + `normalize(front)` dedup, stamps `meta.status`); `installPack` default-skips `meta.status==="needs-review"` (+ `heldForReview`); `resolveMode` (pure, exhaustive unit table); CLI `pack write` / `pack review` (no-LLM); the `recallit-pack` skill rewired to call `pack write` instead of an inline check. Proven end-to-end on the CLI (a hallucinated card is flagged and skipped on install). **Phase 2 live slice shipped + validated:** `src/packgen/author.ts` `runPackAuthor` (mirrors `runSession`; grants `Read`/`Glob`/`Grep`/`WebFetch`/`WebSearch` + MCP `save_source`/`write_pack`; `permissionMode:"bypassPermissions"`; NEVER installs) + CLI `pack <source>` (`--review`/`--dry-run`/`--auto`/`--scope`/`--style`/`--force`, exit 0/2). **Phase 3 source adapters shipped + live-validated:** `prepareSource` classifies file/url/repo/concept and clones git/npm sources to a temp dir (the agent never needs Bash); kind-aware prompt. Live: file 8/8, URL 17/18, concept 25/25 (grounding=web), repo `npm:is-odd` 20/20 — all honest, ~$0.13–0.34 each. **Remaining:** the `pack edit <id>` tweak loop (re-gate via `pack write` + re-install `--force` with the FSRS-reset caveat) + tweak prose (split/merge/regen-audio) — then Phase 4 (non-destructive enhance). Minor tuning: the `unverified-proper-noun` heuristic over-flagged a date once (errs toward review — safe direction).
