...
function buildPrompt(prep: PreparedSource, opts: PackAuthorOptions): string {
  ...
  return [
    ...
    "2. DRAFT cards grounded in that saved text. Each card:",
    "   - front (question/prompt), back (answer), context (optional surrounding sentence), tags (optional).",
    "   - meta.sourceQuote: a VERBATIM span copied from the text you saved (a literal substring). No quote → no card.",
    "   - The answer must be supported by the quote. Do NOT add facts, numbers, or names that aren't in the source.",
    `   Aim for ~15–30 sharp cards. ${DRAFT_NOTE[prep.kind]}`,
